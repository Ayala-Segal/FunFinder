...
def fill_booking_details():
    """Adds records to BOOKING_DETAILS with foreign key validation"""
    path = get_path('booking_details.csv')
   
    # Ensuring existence of foreign keys from parent tables
    ...
    # Check for existing records to avoid Primary Key violations (booking_id + attraction_id)
    existing_pairs = set()
    ...
    with open(path, mode='a', newline='', encoding='utf-8') as f:
        ...
        added = 0
        attempts = 0
        # Limit attempts to prevent infinite loop if unique combinations run out
...
if __name__ == "__main__":
    if not os.path.exists(DATA_DIR):
        ...
        # Sequence matters for data integrity
        fill_users()
        fill_attractions()
