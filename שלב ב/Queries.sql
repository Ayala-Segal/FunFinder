...
-- This query joins attractions with categories and booking tables, but uses LEFT JOIN so that attractions without bookings by the user will still appear.
-- By filtering where booking_id IS NULL, it ensures we only get attractions the user never booked, specifically in the "Adventure" category.
...
-- Purpose: Get Adventure attractions that the user has NOT booked yet
-- This query filters attractions by the "Adventure" category using a subquery, then checks that no booking exists for that attraction by the user.
-- The NOT EXISTS condition ensures that even one matching booking will exclude the attraction from the result.
...
-- This query joins bookings with attractions to get all past bookings of the user.
-- It then filters only those attractions that do not have a matching review by the same user using NOT EXISTS.
...
-- This version uses a LEFT JOIN to try to match reviews to each booked attraction for the user.
-- By filtering where review_id IS NULL, it keeps only attractions that have no review, meaning the user hasn’t reviewed them yet.
...
-- Form A: JOIN version
-- This query joins attractions with categories and difficulty tables to filter based on names instead of IDs.
-- It returns only attractions that belong to the "Family" category and have an "Easy" difficulty level.
...
-- This version retrieves the category_id and difficulty_id using subqueries instead of joins.
-- It then filters attractions directly by those IDs, achieving the same result without explicit joins.
...
-- Form A
-- This query counts how many bookings each attraction has within a specific category.
-- It then compares each count to the maximum count found in a subquery, returning only the most booked attraction(s).
...
-- Form B
-- This version compares each attraction’s booking count against all others using the ALL operator.
-- It returns attractions whose booking count is greater than or equal to every other count in the same category.
...
-- This query groups all bookings of a user by year and counts how many bookings were made in each year.
-- It also calculates total spending by multiplying ticket count with attraction price and summing the results.
...
-- This query joins categories to bookings through attractions and counts how many bookings each category has.
-- It sorts the results by popularity and returns only the top category using LIMIT.
...
-- This query retrieves attraction details and tries to attach related images using a LEFT JOIN.
-- The LEFT JOIN ensures the attraction is returned even if it has no images in the gallery.
...
-- This query returns attractions for which no booking exists in the last 6 months for the user.
-- It uses NOT EXISTS to exclude any attraction that has a recent booking.
...
-- This query counts bookings per attraction within the last 2 months.
-- It sorts by booking count and limits the result to the top 4 most popular attractions.
...
-- This query deletes reviews older than 3 years by comparing the year of creation to the current year.
-- It only removes reviews that belong to a specific user.
...
-- Query 2
-- Purpose: Delete inactive attractions
-- This query removes attractions that have no bookings in the past year.
-- It uses NOT EXISTS to ensure only completely inactive attractions are deleted.
...
-- This query deletes images that are not linked to any existing attraction.
-- It ensures database consistency by removing unused gallery records.
...
-- This query calculates the average rating for an attraction from all its reviews.
-- It updates the attraction’s avg_rating field with the computed value.
...
-- This query updates a booking date only if the current booking is still more than 72 hours away.
-- It prevents last-minute changes by enforcing a time constraint.
...
-- This query identifies users with more than 5 bookings in the past year.
-- It updates their profile (avatar_url) to indicate they are active customers.
